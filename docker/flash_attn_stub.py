...
The module-level __getattr__ catches top-level attributes imported from
`flash_attn` (current API: flash_attn_func,
flash_attn_with_kvcache, flash_attn_varlen_func) and returns a stub for
any additional top-level names. This stub does not provide
`flash_attn.<submodule>` imports.
"""
...
def __getattr__(name):  # PEP 562 — Python 3.7+ module __getattr__.
    def _stub(*args, **kwargs):
        raise NotImplementedError(
            f"flash_attn.{name} is stubbed in the blockquant image. "
            "The quant path never calls into flash-attn; if you've "
            "reached this from an inference workflow, install real "
            "flash-attn explicitly."
        )
    return _stub
