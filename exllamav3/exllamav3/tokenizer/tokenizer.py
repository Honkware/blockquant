...
def _load_hf_tokenizer(path_tokenizer_json: str) -> HFTokenizer:
    """
    Load tokenizer.json. Some Hub exports (e.g. Qwen3) use BPE merges as JSON pairs
    [["a","b"], ...]; the Rust deserializer expects legacy string lines "a b". Convert
    when from_file fails with ModelWrapper / untagged enum errors.
    """
    try:
        return HFTokenizer.from_file(path_tokenizer_json)
    except Exception as e:
        msg = str(e)
        if "ModelWrapper" not in msg and "untagged enum" not in msg:
            raise
        try:
            with open(path_tokenizer_json, encoding="utf-8") as f:
                data = json.load(f)
            model = data.get("model") or {}
            merges = model.get("merges")
            if not isinstance(merges, list) or not merges:
                raise ValueError("missing merges")
            first = merges[0]
            if not isinstance(first, list) or len(first) != 2:
                raise ValueError("merges not pair lists")
            data["model"]["merges"] = [f"{pair[0]} {pair[1]}" for pair in merges]
            return HFTokenizer.from_str(json.dumps(data))
        except Exception:
            raise e
...
class Tokenizer:
    ...
    def __init__(
        ...
        self.tokenizer = _load_hf_tokenizer(self.path_tokenizer_json)
