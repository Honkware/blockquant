...
def load_qwen3_vl_preprocess_config(directory: str) -> dict:
    """
    HF models may ship `preprocessor_config.json` (flat image processor fields) or
    `processor_config.json` with a nested `image_processor` object (Transformers 4.4x+).
    """
    legacy = os.path.join(directory, "preprocessor_config.json")
    bundled = os.path.join(directory, "processor_config.json")
    if os.path.isfile(legacy):
        with open(legacy, encoding="utf8") as f:
            return json.load(f)
    if os.path.isfile(bundled):
        with open(bundled, encoding="utf8") as f:
            data = json.load(f)
        nested = data.get("image_processor")
        if isinstance(nested, dict):
            return nested
        return data
    raise FileNotFoundError(
        f"Expected preprocessor_config.json or processor_config.json (with image_processor) in {directory}"
    )
...
class Qwen3VLConfig(Config):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        read_prep_config = load_qwen3_vl_preprocess_config(self.directory)
