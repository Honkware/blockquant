...
def compile_model(args, model, config, tokenizer):
    ...
    for key, data in extra_tensors.items():
        size = data["n_bytes"]
        if size > max_shard_bytes:
            print(f" !! Warning, unable to fit extra tensor {key} in single shard of {args['shard_size']} MB")
        if current_shard_size + size > max_shard_bytes and current_shard_size > 0:
            current_shard_size = 0
            out_map.append([])
        current_shard_size += size
    ...
    print(f" -- Creating quantization_config.json", flush = True)
    try:
        create_quantization_config_json(out_dir)
    except Exception as e:
        raise RuntimeError(
            "Failed while writing quantization_config.json "
            "(output safetensors are written; check logs above for the underlying error)."
        ) from e

    if owns_qtensors_stc:
        qtensors_stc.close()
